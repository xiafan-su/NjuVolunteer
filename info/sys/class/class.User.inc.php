...
class User extends DB_Connect {
	...
	public function loginFromWebAgent($headers){//从统一认证登录过来
		$common_name = $headers["common-name"];
		...
		if( $results['count'] > 0 ){
			...
			if( $count['count'] == 0 ){//如果login表中不存在该用户，则添加一项纪录
				$sql = "INSERT INTO login(id,password,permission) VALUES ('".$userid."', '', '1')";
				if ( ! mysql_query( $sql, $this->root_conn ) ) {
					die( 'ERROR1:'.mysql_error() );
				}
			}
